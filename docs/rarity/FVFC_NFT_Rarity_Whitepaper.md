<div align="center">
  <img src="https://futurevanityfairclub.com/images/center/traits-8894-hero.webp" alt="FVFC Trait Rarity & Classification Whitepaper" width="100%" />
</div>

# FVFC NFT Rarity & Trait Distribution Whitepaper

> **Project**: Future Vanity Fair Club (FVFC)  
> **Total Supply**: 10,000 Unique Digital Beings (Token #0 ~ #9999)  
> **Trait System**: 881 Unique Trait Values across 16 Primary Trait Types (40 Independent Layers)  
> **Algorithmic Logic**: 2,000+ Generation & Pairing Rules  
> **Dataset File**: [`FVFC_Trait_Rarity_Dataset.csv`](./FVFC_Trait_Rarity_Dataset.csv) (Complete 881-row itemized dataset)  

---

## 1. How Rarity Works

FVFC evaluates rarity across two clear dimensions:

1. **Overall Rarity Rank (Gallery & NFT Pages)**: Your NFT’s total rarity ranking across all 10,000 pieces based on its combined trait scores.
   - 🌟 **Legendary**: Top 1% (Rank #1 ~ #100)
   - 🔥 **Epic**: Top 5% (Rank #101 ~ #500)
   - 💎 **Rare**: Top 20% (Rank #501 ~ #2000)
   - ⚡ **Uncommon**: Top 50% (Rank #2001 ~ #5000)
   - 🎯 **Common**: Standard Tier (Rank #5001 ~ #10000)
2. **Trait Count Tier (Craftsmanship)**: How many individual traits your NFT wears, ranging from 7 to 13 traits across our 40-layer stack.

---

## 2. Trait Count Distribution

Across the 10,000 Vani NFTs, each NFT contains 7 to 13 traits:

| Trait Count | Count (NFTs) | Percentage (%) | Tier Name | Description |
| :--- | :--- | :--- | :--- | :--- |
| **13 Traits** | **3** | **0.03%** | 👑 **Mythic** | Ultra-rare fully equipped grail NFTs |
| **12 Traits** | **38** | **0.38%** | 🌟 **Legendary** | Top-tier complex luxury combinations |
| **11 Traits** | **309** | **3.09%** | 🔥 **Epic** | Premium high-layer collector pieces |
| **10 Traits** | **1,714** | **17.14%** | 💎 **Rare** | Statement looks with rich accessories |
| **9 Traits** | **3,968** | **39.68%** | ⚡ **Prime** | Signature community archetype looks |
| **8 Traits** | **3,248** | **32.48%** | 🎯 **Classic** | Clean and balanced classic styles |
| **7 Traits** | **720** | **7.20%** | 📦 **Minimal** | Pure and minimal foundational looks |
| **Total** | **10,000** | **100.00%** | — | Complete 10,000 collection |

---

## 3. 16 Primary Trait Types (881 Total Values)

| Trait Type | Unique Values | Representative Traits | Rarity Range |
| :--- | :--- | :--- | :--- |
| **Eye** | **182** | WOW, Wink, Metallic Eyebrows, Closed Eyes with Stars, etc. | 0.01% ~ 5.30% |
| **Hair** | **141** | Silver Bob, Afro Hair, Double Buns, Geisha, Headscarf, etc. | 0.01% ~ 5.37% |
| **Cloth** | **130** | Red Velvet Dress, Pink Leopard Bikini, FVFC Suit, Surf Wear, etc. | 0.06% ~ 1.97% |
| **Mouth** | **121** | Open Mouth, Kiss Taupe, Bubble Gum, Prpr, Grin, etc. | 0.01% ~ 21.89% |
| **Right Hand** | **73** | Bitcoin Snap, Champagne, Whisky, Balloons, Matcha Donut, etc. | 0.01% ~ 1.94% |
| **Earring** | **40** | Double Diamond, Emerald, Heart, Amethyst, Crypto, etc. | 0.02% ~ 11.10% |
| **Face Accessories** | **40** | Moto Helmet, Retro Sunglasses, VR Headset, Exotic Crypto Glasses, etc. | 0.01% ~ 7.31% |
| **Left Hand** | **38** | Shiny Guitar, Bass, Butterfly Phone, Cute Cat, Wine Glass, etc. | 0.05% ~ 3.37% |
| **Hat** | **33** | Green Cowboy Hat, Aquamarine Devil Cap, Tennis Cap, Beret, etc. | 0.01% ~ 4.40% |
| **Background** | **27** | Cyber City, Space, Coconut Trees Sunset, Gallery, Crypto, etc. | 0.61% ~ 10.34% |
| **Necklace** | **24** | Butterfly Diamond Choker, Cube Diamond, Golden Chain, etc. | 0.02% ~ 3.35% |
| **Hand Prep** | **14** | Floating Diamond, Yeah Gesture, Crypto Pizza, etc. | 0.09% ~ 3.52% |
| **Head Accessories** | **6** | Halo Headpiece, Crypto Meme Books, Crown, etc. | 0.28% ~ 0.57% |
| **Body Accessories** | **5** | Mermaid Body, Heart Yoga Mat, etc. | 0.07% ~ 0.52% |
| **Skin** | **4** | Ivory (58.23%), Chocolate (21.94%), Luminous (13.88%), Bronze (5.95%) | 5.95% ~ 58.23% |
| **Atmosphere** | **3** | Get Rich (2.36%), Firefly (0.26%), Unicorn (0.16%) | 0.16% ~ 2.36% |
| **Total Summary** | **881 Values** | **16 Primary Trait Types across 40 Layers** | **0.01% ~ 58.23%** |

---

## 4. Notable Rare & Grail Traits

The rarest individual traits across the collection include:

1. **Exotic Crypto Glasses** (Face Accessories) - 1 NFT (0.01%) - 1-of-1 Ultra Grail
2. **Matcha Donut** (Right Hand) - 1 NFT (0.01%) - 1-of-1 Ultra Grail
3. **Pink Leopard Bikini** (Cloth) - 6 NFTs (0.06%)
4. **Unicorn Atmosphere** (Atmosphere) - 16 NFTs (0.16%)
5. **Firefly Atmosphere** (Atmosphere) - 26 NFTs (0.26%)
6. **Mermaid Body** (Body Accessories) - 52 NFTs (0.52%)
7. **Halo Headpiece** (Head Accessories) - 57 NFTs (0.57%)
8. **Get Rich Atmosphere** (Atmosphere) - 236 NFTs (2.36%)
9. **Floating Diamond** (Hand Prep) - 282 NFTs (2.82%)
10. **Bronze Skin** (Skin) - 595 NFTs (5.95%) - Rarest Skin Tier
11. **Luminous Skin** (Skin) - 1,388 NFTs (13.88%) - Special Cyber Glow Skin

---

## 5. Machine-Readable Dataset

The complete 881-row itemized dataset with exact counts, percentages, and rarity tiers is available in:
- 📊 **[`FVFC_Trait_Rarity_Dataset.csv`](./FVFC_Trait_Rarity_Dataset.csv)**

---

## 🌐 Official Verification Links

- **Official Website**: [https://futurevanityfairclub.com](https://futurevanityfairclub.com)
- **Craftsmanship**: [https://futurevanityfairclub.com/craftsmanship](https://futurevanityfairclub.com/craftsmanship)
- **Traits Overview**: [https://futurevanityfairclub.com/traits](https://futurevanityfairclub.com/traits)
- **Collection Gallery**: [https://futurevanityfairclub.com/gallery](https://futurevanityfairclub.com/gallery)
- **Storage Provenance**: [PROVENANCE.md](../../PROVENANCE.md)
